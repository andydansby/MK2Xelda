...
void flick_screen (void) {
	...
	if (p_x <= 0 && ((gpit & sp_LEFT) == 0 || (p_saltando && p_facing == 0)) MAP_BOUNDARY_LEFT) {
		n_pant = SCREEN_LEFT; p_x = 224;
	}
	if (p_x >= 224 && ((gpit & sp_RIGHT) == 0 || (p_saltando && p_facing)) MAP_BOUNDARY_RIGHT) {
		n_pant = SCREEN_RIGHT; p_x = 0;
	}
	if (p_y <= 0 && p_saltando MAP_BOUNDARY_TOP) {
		n_pant = SCREEN_UP; p_y = 144;
	}
	if (p_y >= 144 && (0 == p_saltando || p_cont_salto >= (PHANTOMAS_JUMP_CTR >> 1)) MAP_BOUNDARY_BOTTOM) {
