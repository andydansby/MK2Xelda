...
void
draw_box_sprite (void)
{
  sp_MoveSprAbs (sp_carriable, spritesClip, 0, VIEWPORT_Y + (f_o_yp >> 3),
                 VIEWPORT_X + (f_o_xp >> 3), f_o_xp, f_o_yp);
}
#endif

#ifdef ENABLE_FO_CARRIABLE_BOXES
void
delete_box_sprite (void)
{
...
unsigned char
FO_add (unsigned char x, unsigned char y, unsigned char t)
{
  f_o_t[fo_idx] = t;
  f_o_x[fo_idx] = x;
  f_o_y[fo_idx] = y;
...
void
FO_paint (unsigned char idx)
{
  fo_au = f_o_t[idx];
#ifdef ENABLE_FO_OBJECT_CONTAINERS
  if (fo_au & 128)
    fo_au = flags[fo_au & 127];
#endif
...
#if FT_FIREZONES == 48
  if (fo_au == FT_FIREZONES)
    return;
#endif

#ifdef SHOW_EMPTY_CONTAINER
  draw_coloured_tile_gamearea (f_o_x[idx], f_o_y[idx],
                               fo_au ? fo_au : ITEM_EMPTY);
#else
...
FO_do (void)
{
  ...
  if (f_o_t[fo_it] && attr (f_o_x[fo_it], f_o_y[fo_it] + 1) < 4
...
//scripting calls
              ...
              while (fo_it < MAX_FLOATING_OBJECTS)
                {
                  ...
                  if (f_o_x[fo_it] == fo_x
...

                    {
                      ...
                      if (f_o_t[fo_it] & 128)
                        {
                          // I have to change what's in items[flags [FLAG_ITEM_SELECTED]]
                          // So there are in flags [f_o_t [fo_it] & 127]
  ...
  // Update sprite?
  if (p_hasbox != 99)
    sp_MoveSprAbs (sp_carriable, spritesClip, 0,
                   VIEWPORT_Y + (gpy >> 3) - 1, VIEWPORT_X + (gpx >> 3),
                   gpx & 7, gpy & 7);
#endif
