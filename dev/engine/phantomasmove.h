...
// Copyleft 2014 by The Mojon Twins
...
unsigned char move () {
	...
	if (p_saltando == 0) {
		...
		// Jump?
#if PHANTOMAS_ENGINE == 3
		if (0 == p_saltando && 0 == falling && (gpit & sp_UP) == 0) {
#else
		if ((gpit & sp_UP) == 0) {
#endif
			p_cont_salto = 0;
		...
		// Jump
#if PHANTOMAS_ENGINE == 4
		if (p_cont_salto < (p_jump_max_steps >> 1)) {
#else
		if (p_cont_salto < (PHANTOMAS_JUMP_CTR >> 1)) {
#endif
			// Up
			my = -p_vy;
			check_to_v = WTOP;
...
#if PHANTOMAS_ENGINE == 4
		if (p_cont_salto < p_jump_max_steps) {
#else
		if (p_cont_salto < PHANTOMAS_JUMP_CTR) {
#endif			
			p_cont_salto ++;
		} else {
			p_saltando = 0;
...
#include "engine/playermods/fire_button.h"	
	...
	// Calculate frame
	if (p_saltando || falling) {
		p_next_frame = player_frames [p_facing ? 9 : 8];
	} else {
#ifdef RESET_TO_WHEN_STOPPED
		if (d_pressed) {
			p_next_frame = player_frames [p_facing + p_frame];
		} else {
			p_next_frame = player_frames [p_facing + RESET_TO_WHEN_STOPPED];
		}
#else		
		p_next_frame = player_frames [p_facing + p_frame];
#endif		
	...
	// possee
	possee = (0 == p_saltando && 0 == falling && 0 == p_gotten);
