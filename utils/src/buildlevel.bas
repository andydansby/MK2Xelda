...
Sub usage
	Puts ("buildlevel v 0.2 [MK2]")
	Puts ("usage:")
	Puts ("")
	Puts ("$ buildlevel mapa.map map_w map_h lock font.png work.png spriteset.png enems.ene scr_ini x_ini y_ini max_objs enems_life behs.txt level.bin [decorations.spt]")
	Puts ("")
	Puts ("where:")
	...
	Puts ("     [Use 99 if you #define DEACTIVATE_KEYS]")
	...
	Puts ("   * decorations.spt if specified, maps are forced to 16 tiles + decorations")
	Puts ("")
End Sub
...
Const C_SCRY = 11
Const C_MAXOBJS = 12
Const C_ENEMSLIFE = 13
Const C_BEHSNAME = 14
Const C_OUTPUTFILE = 15
Const C_DECORATIONSNAME = 16
...
If Len (Command (C_DECORATIONSNAME)) > 0 Then 
	Puts ("Output decorations to " & Command (C_DECORATIONSNAME) & "!")
	doForce = -1
End If
...
If doForce Then 
	fExtra = freefile
	Open Command (C_DECORATIONSNAME) For Output as #fExtra
End If
For y = 0 To map_h - 1
	For x = 0 To map_w - 1
		If is_packed Then
...
If doForce Then
	Puts ("    Out of range tiles written to " & Command (C_DECORATIONSNAME))
...
if tile_lock = 99 then
	Puts ("No bolts will be output (you said 99!)")
else
	Puts ("writing bolts...")
	For i = 0 To 31
		d = l (i).np: Put #fout, , d
