...
Sub usage
	...
	puts ("usage")
	puts ("")
	puts ("$ map2bin mapa.map map_w map_h lock map.bin bolts.bin [connections.txt] [force]")
	Puts ("")
	Puts ("where:")
	...
	Puts ("   * force - forces packed, extra tiles are output an map.bin.spt file.")
	Puts ("")
...
Dim As Integer doForce, forceDone, doYawn
