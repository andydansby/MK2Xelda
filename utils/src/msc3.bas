...
Sub stringToArray (in As String)
	...
	For m = 1 To Len (in)
		...
		If (character >= "A" and character <= "Z") or (character >= "0" and character <="9") or character = "#" or character = "_" or character = "'" or character="<" or character=">" or character="!" or character="?" or character = "." or character = "-" or character=":" or character="$" Then
...
Sub autoExpandFlags ()
	Dim As Integer irw
	' This function auto-expands "#X" by "FLAG X" in the first parameter
	' Is worth to not have to put "SET FLAG $ HOLA = 1" and to be able to put "SET $ HOLA = 1"
	' Which seems to me a little less painful and, then, later, more readable.
	
	' In any case it is in LP 1, and when detecting it it is necessary to throw the others a box "back"
	' And put flag on LP 1 and the value on LP 2.
	
	' How many to throw behind? Let's say 10, will cover this long until it casque and
	' I wasted 2 hours wondering why.
	
	' Why not change the QUALITY of this code.
...
Function procesaClausulas (f As integer) As String
	' Read clauses from the open file nPant screen f
	Dim linea As String
	...
	If doIncludeDecos Then
		...
		Open decoInclude For Input as #dF
			' We look for the screen in question
			...
			If estado = 1 Then
				...
				' We create a new IF TRUE clause
				clausula = chr (&HF0)
				clausulasUsed (&HF0) = -1
				...
				' Now we look for the section DECORATIONS
				sc_terminado = 0
				While Not sc_terminado And Not Eof (dF)
					Line input #dF, linea
					linea = Trim (linea, Any chr (32) + chr (9))
					...
					if lP(0) = "DECORATIONS" Then
						' Parse and include a list of prints.
				...
				' We add the length of the clause to the clause
				clausula = Chr (len (clausula)) + clausula

				' And the clause to the list of clauses.
...
Dim As Integer f, f2, f3, i, j, nPant, maxidx, scriptCount, binPt, offsPt
...
if command (1) = "" or  maxidx = 0 then
	print "MojonTwins Scripts Compiler 3.92"
	print "MT Engine MK2 v0.88+"
	Print
	print "uso: msc3 input n_pants [rampage]"
	print ""
	print "input   - archivo de script"
	print "n_pants - num. de pantallas"
	print "rampage - incluye 'rampage' si los scripts irán en otra pagina de RAM"
	print
	system
end if
...
Print #f3, "// Generado por Mojon Script Compiler de la MT Engine MK2"
Print #f3, "// Copyleft 2014 The Mojon Twins"
Print #f3, ""
...
If maxItem > 0 then
	...
	If itemSetOr = 0 Then
		...
		print #f3, "        if (its_it != flags [FLAG_SLOT_SELECTED]) {"
		print #f3, "            sp_PrintAtInv (" & (itemSetY + 2) & ", its_p, 0, 0);"
		print #f3, "            sp_PrintAtInv (" & (itemSetY + 2) & ", its_p + 1, 0, 0);"
		print #f3, "        } else {"
		print #f3, "            sp_PrintAtInv (" & (itemSetY + 2) & ", its_p, " & itemSelectClr & ", " & itemSelectC1 & ");"
		print #f3, "            sp_PrintAtInv (" & (itemSetY + 2) & ", its_p + 1, " & itemSelectClr & ", " & itemSelectC2 & ");"
		print #f3, "        }"
	...
	Else
		...
		print #f3, "        if (its_it != flags [FLAG_SLOT_SELECTED]) {"
		print #f3, "            sp_PrintAtInv (its_p + 2, " & itemSetX & ", 0, 0);"
		print #f3, "            sp_PrintAtInv (its_p + 2, " & (itemSetX + 1) & ", 0, 0);"
		print #f3, "        } else {"
		print #f3, "            sp_PrintAtInv (its_p + 2, " & itemSetX & ", " & itemSelectClr & ", " & itemSelectC1 & ");"
		print #f3, "            sp_PrintAtInv (its_p + 2, " & (itemSetX + 1) & ", " & itemSelectClr & ", " & itemSelectC2 & ");"
		print #f3, "        }"
		print #f3, "        its_p += " & itemSetStep & ";"
		print #f3, "    }"
		print #f3, "}"
...
Print #f3, "    #endasm"
Print #f3, "#endif"
Print #f3, "    return sc_b;"
Print #f3, "}"
Print #f3, ""
Print #f3, "unsigned char __FASTCALL__ read_vbyte (void) {"
Print #f3, "    sc_c = read_byte ();"
Print #f3, "    return (sc_c & 128) ? flags [sc_c & 127] : sc_c;"
Print #f3, "}"
Print #f3, ""
Print #f3, "void __FASTCALL__ readxy (void) {"
Print #f3, "    sc_x = read_vbyte ();"
Print #f3, "    sc_y = read_vbyte ();"
Print #f3, "}"
Print #f3, ""
Print #f3, "#if !(defined (PHANTOMAS_ENGINE) || defined (HANNA_ENGINE))"
print #f3, "void __FASTCALL__ stop_player (void) {"
Print #f3, "    p_vx = p_vy = 0;"
Print #f3, "}"
Print #f3, "#endif"
Print #f3, ""
Print #f3, "void __FASTCALL__ reloc_player (void) {"
print #f3, "#if defined (PHANTOMAS_ENGINE) || defined (HANNA_ENGINE)"
Print #f3, "    p_x = read_vbyte () << 4;"
Print #f3, "    p_y = read_vbyte () << 4;"
print #f3, "#else"
Print #f3, "    p_x = read_vbyte () << 10;"
Print #f3, "    p_y = read_vbyte () << 10;"
...
Print #f3, "    asm_int [0] = main_script_offset + whichs + whichs;"
Print #f3, ""
Print #f3, "#ifdef MODE_128K"
Print #f3, "    #asm"
Print #f3, "        di"
...
Print #f3, "        ld a, (hl)"
Print #f3, "        ld (_asm_int_2), a"
Print #f3, "        inc hl"
Print #f3, "        ld a, (hl)"
Print #f3, "        ld (_asm_int_2 + 1), a"
Print #f3, "    #endasm"
Print #f3, ""
Print #f3, "    script = (unsigned char *) (asm_int_2 [0]);"
Print #f3, ""
Print #f3, "#ifdef MODE_128K"
Print #f3, "    #asm"
Print #f3, "        ld b, 0"
...
if clausulasUsed (&H20) Then
	...
	print #f3, "                    readxy ();"
	print #f3, "#if defined (PHANTOMAS_ENGINE) || defined (HANNA_ENGINE)"
	print #f3, "                    sc_terminado = (!(p_x >= (sc_x << 4) - 15 && p_x <= (sc_x << 4) + 15 && p_y >= (sc_y << 4) - 15 && p_y <= (sc_y << 4) + 15));"
	print #f3, "#else"
	print #f3, "                    sc_terminado = (!((p_x >> 6) >= (sc_x << 4) - 15 && (p_x >> 6) <= (sc_x << 4) + 15 && (p_y >> 6) >= (sc_y << 4) - 15 && (p_y >> 6) <= (sc_y << 4) + 15));"
	print #f3, "#endif"
	print #f3, "                    break;"
end if
...
if clausulasUsed (&H21) Then
	...
	print #f3, "#else"
	print #f3, "                    sc_terminado = (!((p_x >> 6) >= sc_x && (p_x >> 6) <= sc_y));"
	print #f3, "#endif"
	print #f3, "                    break;"
end if
...
if clausulasUsed (&H22) Then
	...
	print #f3, "#else"	
	print #f3, "                    sc_terminado = (!((p_y >> 6) >= sc_x && (p_y >> 6) <= sc_y));"
	print #f3, "#endif"
	print #f3, "                    break;"
end if
...
if actionsUsed (&H20) Then
	...
	print #f3, "                        update_tile (sc_x, sc_y, behs [sc_n], sc_n);"
	print #f3, "                        break;"
End If
...
If actionsUsed (&H6E) Then
	...
	print #f3, "                            update_tile (sc_x, sc_y, map_attr [sc_c], map_buff [sc_c]);"
...
if actionsUsed (&HE0) Then
	...
	print #f3, "#else"
	print #f3, "                        peta_el_beeper (read_vbyte ());"
	print #f3, "#endif"
	print #f3, "                        break;"
End If
...
If actionsUsed (&HEA) Then
	...
	print #f3, "#else"
	print #f3, "                        p_safe_x = p_x >> 10;"
	print #f3, "                        p_safe_y = p_y >> 10;"
	print #f3, "#endif"
	print #f3, "                        break;"	
End If
...
if actionsUsed (&HF4) Then
	...
	print #f3, "                            sc_n = read_byte ();"
	print #f3, "                            update_tile (sc_x >> 4, sc_x & 15, behs [sc_n], sc_n);"
	print #f3, "                        }"
	print #f3, "                        break;"
End If
